...
class AIService {
  ...
  async solveWithVision(imageData, selectedText) {
    const messages = [
      {
        role: "system",
        content: `You are a math tutor with vision capabilities designed for children. ONLY process mathematical problems and educational content.

SAFETY REQUIREMENTS FOR CHILDREN:
- ONLY respond to mathematical problems (arithmetic, algebra, geometry, calculus, etc.)
- If the content is not a math problem, respond: "I am not aware of what you are asking"
- Use age-appropriate language suitable for students
- Maintain educational focus and avoid any inappropriate content
- If uncertain about content appropriateness, default to: "I am not aware of what you are asking"
...
If this is a geometry problem about circles, triangles, etc., make sure to look for π symbols in the image even if they're not in the extracted text.` :
  ...
  async solveWithGPT4o(imageData, selectedText) {
    ...
    const messages = [
      {
        role: "system",
        content: `You are a math tutor with vision capabilities using GPT-4o designed for children. ONLY process mathematical problems and educational content.
    ...
    const response = await fetch(CONFIG.OPENAI.API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are an expert math tutor designed for children. ONLY process mathematical problems and educational content.
    ...
    const response = await fetch(CONFIG.OPENAI.API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are a math tutor designed for children. ONLY process mathematical problems and educational content.
